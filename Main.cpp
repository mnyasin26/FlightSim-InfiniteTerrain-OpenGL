...
int main()
{
	...
	if (!success) return -1;
	Shader forPlane("PlaneShad");
	forPlane.attachShader("shaders/terrain.vert");
	forPlane.attachShader("shaders/terrain.frag");
	forPlane.linkPrograms();

	


	Model plane("resources/seahorse.obj");
	...
	while (window.continueLoop())
	{
		...
		forPlane.setMat4("projection", projection);
		forPlane.setMat4("view", view);

		// render the loaded model
		glm::mat4 model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.0f)); // translate it down so it's at the center of the scene
		model = glm::scale(model, glm::vec3(1.0f, 1.0f, 1.0f));	// it's a bit too big for our scene, so scale it down
		forPlane.setMat4("model", model);
		plane.Draw(forPlane);
